<script lang="ts">
	...
	import { getSpec, updateSpec, type TopicInput } from '$lib/api';
	...
	onMount(async () => {
		try {
			const spec = await getSpec(specCode);
	...
	async function handleSubmit() {
		...
		try {
			...
			errorMsg = err.message || 'Failed to update specification.';
		} finally {
			isSubmitting = false;
		}
	}
...
<svelte:head>
	<title>Edit Specification - Topic Tracker</title>
</svelte:head>
...
<main class="page-content create-spec-page">
	...
	<p>Update the specification details, topics and subtopics.</p>
	...
	{:else}
		...
		<div class="form-section">
			...
			<div class="form-grid">
				...
				<div class="form-field">
					<label for="spec-code">Spec Code</label>
					<input id="spec-code" type="text" value={specCode} disabled />
				</div>
			</div>
			<div class="form-field">
